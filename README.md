...
当前阶段：**单板音频采集、波形显示与测距结果 UI 已有实现；LwIP 已接入，开发者已验证 ping 成功；双板应用通信、时钟同步和距离计算待开发。**
...
| 以太网/LwIP | 静态 IP 192.168.10.10/24，主循环处理收包和超时 | 2026-09-18 开发者确认 ping 成功；双板业务通信及长期稳定性待验收 |
| TIM5 | 1 MHz、32 位计时配置，当前启动流程调用初始化 | 尚未找到计数启动调用；未实现同步时钟 |
...
TIM5 已保存 `Prescaler=99`、`Period=0xFFFFFFFF` 的配置，在当前时钟树下对应 1 MHz（1 us/计数）。当前 `LCD_GRID_DEMO_ONLY=0`，启动流程调用 `MX_TIM5_Init()`，但初始化不等于启动，尚未找到 TIM5 计数启动调用，也未实现跨板同步。
