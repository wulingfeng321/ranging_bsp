...
void MicScope_Init(void)
{
  ...
  {
    Message("MIC INIT ERROR: WM8994 / I2C");
    return;
  }
  ...
  {
    Message("MIC START ERROR: SAI / DMA");
    return;
  }
...
{
  int32_t offset = (sample - mean) * 48 / scale;
...
static void DrawChannel(uint32_t channel, int32_t mean, int32_t scale)
{
  ...
  uint16_t top = (uint16_t)(channel * 136U);
  int32_t center = top + 78;
  ...
  BSP_LCD_DisplayStringAt(0, top, (uint8_t *)(channel == 0 ?
      "MIC L | 16k PCM16 | 160ms | AUTO" :
      "MIC R | same scale | 20ms/div"), LEFT_MODE);
  BSP_LCD_SetTextColor(0xFF404040U);
  for (x = 0; x < PLOT_WIDTH; x += 60)
    BSP_LCD_DrawVLine((uint16_t)x, top + 24, 109);
  for (i = top + 30; i <= top + 126; i += 24)
    BSP_LCD_DrawHLine(0, (uint16_t)i, PLOT_WIDTH);
  BSP_LCD_SetTextColor(LCD_COLOR_WHITE);
  BSP_LCD_DrawHLine(0, (uint16_t)center, PLOT_WIDTH);
  BSP_LCD_DrawHLine(0, top + 135, PLOT_WIDTH);
...
void MicScope_Process(void)
{
  ...
  if (!started) return;
  now = HAL_GetTick();
  if (seenBlocks != micDmaBlocks)
  {
    seenBlocks = micDmaBlocks;
    lastReceived = now;
  }
  if (micErrors || haudio_in_sai.ErrorCode || now - lastReceived > 1000U)
  {
    Message("MIC DMA ERROR / NO DATA: check IRQ");
    started = 0;
    return;
  }
  if (validFrames < WINDOW_FRAMES || now - lastDraw < REFRESH_MS ||
